    ...
    loop {
        match queue.pop_front() {
            Some(ref a @ Key { three: Some(_), .. }) => {
                ...
                // if there where not enough keys calculated
                while queue.len() <= QUEUE_SIZE && !has {
                    let next = key_gen.next_stretched();
                    has = a.pair(&next);
            ...
            _ => {
                queue.push_back(key_gen.next_stretched());
                continue;
            }
        }
    }
...
impl KeyGen<'_> {
    ...
    fn next_stretched(&mut self) -> Key {
        loop {
            // check until we get a potential start / end
            self.buffer.clear();
            let key = Key::with_buffer_stretched(self.idx, self.salt, &mut self.buffer);
            self.idx += 1;
            if let Some(key) = key {
                break key;
            }
...
impl Key {
    ...
    fn with_buffer_stretched(idx: usize, input: &str, mut tmp: &mut String) -> Option<Self> {
        tmp.clear();
        let _ = write!(&mut tmp, "{}{}", input, idx);
    ...
    fn check_key(idx: usize, dig: &md5::Digest) -> Option<Self> {
        let mut digest = [0; 32];
